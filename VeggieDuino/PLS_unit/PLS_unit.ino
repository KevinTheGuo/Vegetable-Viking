...
// our struct for getting MPU data
struct MPUPackage{
  float pitch;
  float yaw;
  bool streakActive;
...
void setup() 
{
  ...
  Serial.println(F("This is the PLS unit"));
...
void loop() 
{ 
  // check if we've recieved data!
  if(PLSwork.receiveData())
  {
    Serial.print("pitch is ");
    Serial.print(serialPackage.pitch);
    Serial.print("     yaw is ");
    Serial.print(serialPackage.yaw);
    Serial.print("      streak is ");
    Serial.println(serialPackage.streakActive);
  }
  // now we send out info!
  Serial.print("doop de doo");
